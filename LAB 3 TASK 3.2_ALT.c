...
float scaled; // scaled variable for scaling the ADC values
...
float* array_ptr = storage; // pointer to array, initialised to the array 
//-------------------------------------------------------------------------------------
	...
	switch (testing)
		{
			case POTENTIOMETER:
				...
				GPIOE->BSRRL = (ADC_read()) << 11; // turn ON Leds by shifting bits, the ADC is scaled to 5-bits 
			...
			case ENCODER:
				...
				GPIOE->BSRRL = (int)(fabs(encoderCount/scaler2)) << 11; // turn on LEDs by shifting bit to match PE11-15	
			...
			case COMBINED_TEST:
				...
				if(!(GPIOE->IDR & 0x400)) // turn ON Led PE.10 if OFF
				{
					...
					{
						sum += storage[i];// scale the array values to fit 5-bit 
					}

					avg = ((sum/5)+fabs(encoderCount/scaler2))/2; // sum/5 = avg of POT , (avg of POT + encoder)/2
