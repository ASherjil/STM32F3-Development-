...
int counter = 0;
bool startup = true; // starting flag 
...
static volatile int current_state[2] = {0,0}; // index 0 = CHA, index 1= CHB
static volatile int last_state[2] = {0,0};// index 1 = CHB, index 1 = CHB 
//-------------------------------------------------------------------------
...
void EXTI0_IRQHandler() // external interrupt channel 0 
{
	if (EXTI->PR & EXTI_PR_PR0) // check source
	{
		...
		++counter1;
		encoder_pos();
		if (abs(encoderCount)>15)
		{encoderCount=0;}// reset when max 4-bit value is reached
